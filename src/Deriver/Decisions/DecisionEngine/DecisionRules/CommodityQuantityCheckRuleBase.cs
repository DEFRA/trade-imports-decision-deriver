...
public readonly record struct ChedQuantityCommodity(string? CommodityCode, decimal? Weight, decimal? Quantity);

public abstract class CommodityQuantityCheckRuleBase : DecisionRule
{
    protected abstract string? GetChedType(DecisionEngineContext context);

    protected abstract IEnumerable<ChedQuantityCommodity> GetChedCommodities(DecisionEngineContext context);

    protected virtual void OnCompleted(DecisionEngineContext context) { }
    ...
    protected override DecisionEngineResult DoExecute(DecisionEngineContext context, DecisionRuleDelegate next)
    {
        ...
        var chedCommodities = GetChedCommodities(context)
    ...
    private static ValidationResult Validate(
        ...
        List<ChedQuantityCommodity> chedCommodities
    )
    {
    ...
    {
        var chedType = GetChedType(context);
    ...
    private DecisionEngineResult? AddPassiveResult(
        DecisionEngineResult result,
        DecisionInternalFurtherDetail furtherDetail
    )
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static bool WeightValid(
        string mrn,
        Commodity commodity,
        List<Commodity> mrnCommodities,
        List<ChedQuantityCommodity> chedCommodities,
