...
public class SqsHealthCheck(IConfiguration configuration) : IHealthCheck
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _ = await client.GetQueueUrlAsync(configuration.GetValue<string>("DATA_EVENTS_QUEUE_NAME"), cancellationToken).ConfigureAwait(false);
    ...
    private IAmazonSQS CreateSqsClient()
    {
        ...
        {
            return new AmazonSQSClient(new BasicAWSCredentials(clientId, clientSecret),
                new AmazonSQSConfig
                {
